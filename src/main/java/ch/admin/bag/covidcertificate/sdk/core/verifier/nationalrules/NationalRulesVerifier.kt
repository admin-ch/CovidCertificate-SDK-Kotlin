...
internal class NationalRulesVerifier {
	...
	private val validityRangeCalculator = ValidityRangeCalculator()
	...
	fun verify(dccCert: DccCert, ruleSet: RuleSet, clock: Clock = Clock.systemUTC()): CheckNationalRulesState {
		...
		for (rule in ruleSet.rules) {
			...
			if (!isSuccessful) {
				return getErrorStateForRule(rule, dccCert, ruleSet)
			}
		}

		val validityRange = getValidityRange(dccCert, ruleSet)
	...
	private fun getErrorStateForRule(
		...
		return when (rule.id) {
			...
			"VR-CH-0004" -> getValidityRange(dccCert, ruleSets)?.let {
			...
			"VR-CH-0005" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_YET_VALID(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			"VR-CH-0006" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_VALID_ANYMORE(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			"VR-CH-0007" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_YET_VALID(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			...
			"TR-CH-0005" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_YET_VALID(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			"TR-CH-0006" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_VALID_ANYMORE(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			"TR-CH-0007" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_VALID_ANYMORE(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			...
			"RR-CH-0002" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_YET_VALID(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
			"RR-CH-0003" -> getValidityRange(dccCert, ruleSets)?.let {
				CheckNationalRulesState.NOT_VALID_ANYMORE(it, rule.id)
			} ?: CheckNationalRulesState.INVALID(NationalRulesError.VALIDITY_RANGE_NOT_FOUND, rule.id)
	...
	private fun getValidityRange(dccCert: DccCert, ruleSet: RuleSet): ValidityRange? {
		return validityRangeCalculator.getValidityRange(dccCert, ruleSet.validityRules, ruleSet.valueSets)
	}
}
