...
class ValidityRangeCalculatorTest {
    ...
    fun testOneDoseVaccinationValidityRange() {
        ...
        val validityRange = validityRangeCalculator.getDisplayValidityRangeForSystemTimeZone(nationalRuleSet.displayRules, data, CertType.VACCINATION)
    ...
    fun testRatTestValidityRange() {
        ...
        val validityRange = validityRangeCalculator.getDisplayValidityRangeForSystemTimeZone(nationalRuleSet.displayRules, data, CertType.TEST)
        assertNotNull(validityRange)
        ...
        val expectedValidFrom = sampleCollectionTime.atZoneSameInstant(ZoneId.systemDefault()).toLocalDateTime().truncatedTo(ChronoUnit.MILLIS)
        assertEquals(expectedValidFrom, validityRange?.validFrom)

        val expectedValidUntil = sampleCollectionTime.plusHours(48).atZoneSameInstant(ZoneId.systemDefault()).toLocalDateTime().truncatedTo(ChronoUnit.MILLIS)
    ...
    fun testPcrTestValidityRange() {
        ...
        val expectedValidUntil = sampleCollectionTime.plusHours(72).atZoneSameInstant(ZoneId.systemDefault()).toLocalDateTime().truncatedTo(ChronoUnit.MILLIS)
        assertEquals(expectedValidUntil, validityRange?.validUntil)
    }
    ...
    fun testRecoveryValidityRange() {
        ...
        val validityRange = validityRangeCalculator.getDisplayValidityRangeForSystemTimeZone(nationalRuleSet.displayRules, data, CertType.RECOVERY)
        assertNotNull(validityRange)
